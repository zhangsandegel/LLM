...
class GRPOArguments:
    ...
    batch_size = 1
      # === 新增：采样明细保存为 JSONL ===
    save_jsonl = True
    samples_jsonl = '/home/work/zhangyu/res/Qwen2-1.5B-GRPO/samples.jsonl'
...
class GRPOTrainer:
    ...
    def __init__(self,
        ...
        if getattr(self.args, "save_jsonl", False) and getattr(self.args, "samples_jsonl", None):
            os.makedirs(os.path.dirname(self.args.samples_jsonl), exist_ok=True)
        self._t0 = time.time()
    ...
    def generate_experiences(self, inputs):
        ...
        # === 新增：把本组(G条)的采样写入 JSONL ===
        records = []
        # 每条 response 的一些数值要转 float，tensor 需要 .item() / .tolist()
        for j in range(self.args.num_generations):
            rec = {
                "prompt": prompt,
                "response": response_texts[j],
                "answer": answer,
                "reward_per_func": [float(x) for x in rewards_per_func[:, j].nan_to_num(0.0).tolist()],
                "reward_total": float(torch.nan_to_num(rewards[j], 0.0).item()),
                "advantage": float(torch.nan_to_num(advantages[j], 0.0).item()),
                # 可选：记录一些概率学信息（均值即可，避免太大）
                "old_logp_mean": float(torch.nan_to_num(old_action_log_probs[j].mean(), 0.0).item()),
                # 可选：长度信息
                "response_len": int(samples.response_length[j].item()) if isinstance(samples.response_length, torch.Tensor) else int(samples.response_length),
            }
            records.append(rec)
    ...
    def _append_jsonl(self, records: List[dict]):
        if not getattr(self.args, "save_jsonl", False) or not getattr(self.args, "samples_jsonl", None):
            return
        # 仅主进程写，避免多卡重复
        if self.accelerator is not None and not self.accelerator.is_main_process:
            return
        with open(self.args.samples_jsonl, "a", encoding="utf-8") as f:
            for r in records:
                # 规避 NaN/Inf
