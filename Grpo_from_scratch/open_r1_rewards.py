...
# 该函数旨在评估给定完成内容（completions）与标准答案（solution）之间的匹配度，并返回一个奖励值列表
def accuracy_reward(completions: list[list[dict[str, str]]], solution: list[str], **kwargs) -> list[Optional[float]]:
    """Reward function that checks if the completion is the same as the ground truth."""
    # 提取内容：从completions中提取每个完成内容的文本，存储在contents列表中。
    contents = [completion[0]["content"] for completion in completions]
    # 创建一个空列表rewards来存储每个完成内容的奖励值。
    rewards = []
    # 遍历和评估：对于每个完成内容和对应的标准答案：
...
# 该函数旨在作为一个奖励机制，用于检查给定的文本内容是否遵循了特定的格式规则
# 具体来说，它检查文本中的推理过程是否被<think>和</think>标签包围，同时最终答案是否被<answer>和</answer>标签包围。这种格式通常用于结构化地展示AI的推理过程和最终答案，以便于区分和理解
def format_reward(completions, **kwargs):
    """Reward function that checks if the reasoning process is enclosed within <think> and </think> tags, while the final answer is enclosed within <answer> and </answer> tags."""
    pattern = r"^<think>\n.*?\n</think>\n<answer>\n.*?\n</answer>$"
    # 使用列表推导式从completions列表中提取每个完成文本的内容。这里假设每个完成文本是一个包含字典的列表，且字典中有一个键为"content"的项，其值是文本内容
    completion_contents = [completion[0]["content"] for completion in completions]
    matches = [re.match(pattern, content, re.DOTALL | re.MULTILINE) for content in completion_contents]
    # 中每个元素对应于一个完成文本的匹配结果。如果文本内容匹配了指定的模式，则对应的元素为1.0，表示获得了奖励；如果不匹配，则为0.0，表示没有获得奖励。
...
def get_reward_funcs(script_args) -> list[Callable]:
    # 奖励函数注册表：
    '''
        REWARD_FUNCS_REGISTRY是一个字典，它映射了字符串键到对应的奖励函数。这些奖励函数包括：
        accuracy_reward：基于准确性的奖励函数。
        format_reward：基于格式的奖励函数。
        reasoning_steps_reward：基于推理步骤的奖励函数。
        get_cosine_scaled_reward(...)：一个通过参数动态生成的余弦相似度奖励函数。
            功能：
                通过余弦相似度计算生成文本与标准答案的相似性，并根据配置参数动态缩放奖励值。
            参数：
                min_value_wrong/max_value_wrong：错误答案的最小/最大奖励范围。
                min_value_correct/max_value_correct：正确答案的最小/最大奖励范围。
                max_len：允许的最大文本长度（超出可能惩罚）。
            用途：
                适用于需要量化文本语义相似度的场景（如问答系统）。
        get_repetition_penalty_reward(...)：一个通过参数动态生成的重复惩罚奖励函数。
            功能：
                检测文本中重复的 N-gram（如短语或单词组合），并施加惩罚。
            参数：
                ngram_size：检查重复的 N-gram 大小（如 2 表示检查连续 2 个词的重复）。
                max_penalty：最大惩罚值（负奖励）。
            用途：
                避免模型生成重复内容（如循环输出同一句话）。
        len_reward：基于长度的奖励函数。
            功能：
                根据文本长度进行奖励或惩罚。
            参数：
                max_len：允许的最大文本长度（超出可能惩罚）。
        code_reward、binary_code_reward、ioi_code_reward、cf_code_reward：这些都是基于代码评估的奖励函数，通过partial和update_wrapper进行了参数绑定和函数包装。
            功能：
                动态生成一个部分绑定的 code_reward 函数，用于评估生成代码的执行结果。
            参数：
                num_parallel：并行执行代码的进程数。
                provider_type：代码执行的后端服务（如本地 Docker 或云 API）。
                enforce_same_language：是否强制代码与预期语言一致。
            用途：
                评估代码生成任务（如编程题解答）的正确性。
        get_code_format_reward(...)：一个通过参数动态生成的代码格式奖励函数。
            功能：
                检查生成代码的格式（如缩进、括号匹配）是否符合指定语言的规范。
            参数：
                language：目标编程语言（如 Python）。
            用途：
                提升生成代码的可读性。
        tag_count_reward：基于标签计数的奖励函数。
            功能：
                统计生成文本中特定标签（如 <answer>）的数量并给予奖励。
            用途：
                确保输出符合结构化要求（如 XML/HTML 标签）。
        get_soft_overlong_punishment(...)：一个通过参数动态生成的软性超长惩罚函数。
            功能：
                对超过最大长度的文本施加渐进式惩罚（非硬性截断）。
            参数：
                max_completion_len：允许的最大长度。
                soft_punish_cache：是否缓存惩罚计算结果。
            用途：
                平衡生成内容的长度与质量。
    '''
    REWARD_FUNCS_REGISTRY = {
        ...
        # cosine 余弦相似度奖励，通过余弦相似度计算生成文本与标准答案的相似性，并根据配置参数动态缩放奖励值。
