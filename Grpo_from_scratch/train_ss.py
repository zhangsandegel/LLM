...
'''
存储一次 group（对一个 prompt 生成 num_generations 个候选）的所有相关张量：

prompt_response_ids: Tensor, shape (num_generations, seq_len) —— 包含 prompt + model 生成的 response 的 token ids（已 pad 到统一长度）。

response_ids: (num_generations, response_len) —— response 的 token ids（切出 prompt 后的部分）。

attention_mask: 与 prompt_response_ids 对齐，1/0 表示有效 token。

action_mask: 对应 response_ids 的有效 token，1 表示该位置是生成 token，0 表示 pad 或 eos。

num_actions / response_length：生成序列长度信息。
'''
@dataclass
class Samples:
