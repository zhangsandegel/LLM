...
- **目标**（无 critic 的 PPO 变体）：
  ...
  $\mathcal{L}_{\text{GRPO}} = -\mathbb{E}\Big[A_i \cdot \sum_t \log \pi_\theta(a_t|s_t)\Big] + \beta\, \mathrm{KL}\!\left(\pi_\theta \,\|\, \pi_{\text{ref}}\right)$
...
- **主项（剪切）：**
  ...
  $\mathcal{L}^{\text{clip}} = \mathbb{E}\big[\min(r_t A_t,\ \text{clip}(r_t,1-\epsilon,1+\epsilon)A_t)\big]$
